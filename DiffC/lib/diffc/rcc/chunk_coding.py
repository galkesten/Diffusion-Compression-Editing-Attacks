...
def chunk_and_encode(mu, chunk_sizes, shared_seed=0, timestep=None):
    partition_indices = partition_mu(len(mu), chunk_sizes, shared_seed)

    partitions = []
    seeds = []

    # print("num_chunks:", len(chunk_sizes))
    # print("Ks", [int(2 ** chunk_size) for chunk_size in set(chunk_sizes)])
    ...
    for i, chunk_size in enumerate(chunk_sizes):
        ...
        # data.append({'step_index' : shared_seed, 'chunk_index' : i, 'K' : int(2 ** chunk_size),
        #              'chunk_size' : chunk_mask.sum(), 'chunk_bits' : chunk_size, 'num_chunks' : len(chunk_sizes),
        #              'timestep' : timestep})
        # print('csv!')
        
        # filename = 'numbers.csv'
        # data = [{'step' : shared_seed, 'rel' : seed/(2**chunk_size)}]
        ...
        # else:
            ...
            # new_data.to_csv(filename, mode='w', index=False, header=True)
        
        seeds.append(seed)
        partitions.append(partition)
    
    # filename = 'steps_chunks.csv'
