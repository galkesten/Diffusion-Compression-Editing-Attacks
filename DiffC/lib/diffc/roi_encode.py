...
def roi_encode(
    ...
    for step_index, prev_timestep in tqdm(enumerate(timestep_schedule), total=len(timestep_schedule)):  # "previous" as in closer to 1 than the current snr
        ...
        masking = (q_mu - p_mu) * mask
        q_mu_flat_normed = (masking / std).flatten().detach().cpu().numpy()
